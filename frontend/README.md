# Frontend — QualiBank WebScraper (guia para iniciantes)
...
```ts
export async function consultarBeneficio(dados: ConsultaDados): Promise<ConsultaResultado> {
...
### 3.7 `components/resultado.tsx` — cards do resultado

Transforma o JSON em cards (via `utils/resultado.ts`) e também exibe o **tempo de execução** da consulta no topo (ex.: `Tempo de execução: 12.3s`), usando o campo `executionTimeMs` que o backend retorna.

### 3.6 `components/` — interface

- `consulta-form.tsx` — o formulário em si (view pura, usa o hook).
- `resultado.tsx` — transforma o JSON em cards (a lógica de transformação fica em `utils/resultado.ts`).
